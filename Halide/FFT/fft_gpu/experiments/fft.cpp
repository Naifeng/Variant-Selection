...
ComplexFunc fft2d_r2c(Func r,
    ...
    if (skip_zip) {
        ...
        result.vectorize(n0, std::min(N0, target.natural_vector_size(result.output_types()[0]))); // TODO
    ...
    std::cout << "zip_width: " << zip_width;
    //int zip_width = 32; // TODO
    ...
    std::cout << "zip_width after gcd: " << zip_width << std::endl;
    ...
    // a race condition.
    dft.update(1).allow_race_conditions().vectorize(n0z1, p3); // 1
    dft.update(2).allow_race_conditions().vectorize(n0z2, p4); // 2 
    dft.update(4).allow_race_conditions().vectorize(n0z1, p5); // 3 
    dft.update(5).allow_race_conditions().vectorize(n0z2, p6); // 4

    std::cout << "update parameters: " << p3 << p4 << p5 << p6 << std::endl;
...
ComplexFunc fft2d_r2c(Func r,
                      int N0, int N1,
                      const Target &target,
                      const Fft2dDesc &desc, int p1, int p2, int p3, int p4, int p5, int p6) {
    return fft2d_r2c(r, radix_factor(N0), radix_factor(N1), target, desc, p1, p2, p3, p4, p5, p6);
}
